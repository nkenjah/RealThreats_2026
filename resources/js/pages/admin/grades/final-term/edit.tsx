...
export default function FinalTermEdit({
    finalTermGrade,
    enrollments,
    courseOfferings,
}: Props) {
    const { data, setData, patch, processing, errors } = useForm({
        enrollment_id: String(finalTermGrade.enrollment_id),
        course_offering_id: String(finalTermGrade.course_offering_id),
        total_score: String(finalTermGrade.total_score),
        letter_grade: finalTermGrade.letter_grade,
        gpa_points: String(finalTermGrade.gpa_points ?? ''),
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(`/admin/final-term-grades/${finalTermGrade.id}`);
    };

    return (
        <>
            <Head title="Edit Final Grade" />
                ...
                <div className="max-w-lg rounded-lg border bg-card p-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <Button type="submit" disabled={processing}>
                            {processing ? 'Updating...' : 'Update Final Grade'}
                        </Button>
                    </form>
                </div>
            </div>
