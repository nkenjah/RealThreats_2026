...
export default function DegreeAuditsEdit({
    degreeAudit,
    students,
    programs,
}: Props) {
    const { data, setData, patch, processing, errors } = useForm({
        student_id: String(degreeAudit.student_id),
        program_id: String(degreeAudit.program_id),
        total_credits_required: String(degreeAudit.total_credits_required),
        total_credits_earned: String(degreeAudit.total_credits_earned),
        status: degreeAudit.status,
        generated_at: degreeAudit.generated_at,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(`/admin/academics/degree-audits/${degreeAudit.id}`);
    };

    return (
        <>
            <Head title="Edit Degree Audit" />
            <div className="space-y-6 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="text-2xl font-bold">Edit Degree Audit</h1>
                </div>
                ...
                <div className="max-w-lg rounded-lg border bg-card p-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <Button type="submit" disabled={processing}>
                            {processing ? 'Updating...' : 'Update Degree Audit'}
                        </Button>
                    </form>
                </div>
            </div>
